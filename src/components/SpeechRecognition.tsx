...
const SpeechRecognition: React.FC<SpeechRecognitionProps> = ({ 
  ...
  useEffect(() => {
    ...
    if (isElectron) {
      ...
      window.electron?.on('speech:transcription', (text: string, isFinal: boolean) => {
        setTranscript(prevText => isFinal ? text : prevText + ' ' + text);
  ...
  useEffect(() => {
    const handleTranscriptUpdate = (
      ...
      console.log('IPC transcript update:', data);
      // Update transcript state - overwrite interim results, append final?
      // Simple approach: always show the latest transcript received
      setTranscript(data.transcript);
      if (data.isFinal) {
          // Maybe add a newline or store final segments separately?
          console.log('Final transcript segment received.');
      }
    };
    ...
    <div className="speech-recognition">
      ...
      <div className="transcript-container">
        ...
        <div className="transcript">
          {transcript || <span className="placeholder">Transcription will appear here...</span>}
        </div>
      </div>
    </div>
  );
